# yap-sum — Privacy Policy

_Last updated: 2026-07-10. Publisher: **nalg**._

> Replace "yap-sum" throughout with the final add-on name once chosen.

**Short version: yap-sum has no servers and collects nothing for its developer.
The only place your data goes is the LLM endpoint _you_ configure.**

## What yap-sum does

yap-sum adds a "Summarize" button to YouTube watch pages. When you click it, the
...
## Data yap-sum handles, and where it goes
...
processed under that provider's own privacy policy and terms.** yap-sum has no
...
- **`webRequest`** — to read YouTube's own transcript/caption network responses
  (the reliable way to obtain a transcript). yap-sum does not modify page content
